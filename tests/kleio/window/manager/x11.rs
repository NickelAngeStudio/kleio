...
use olympus_kleio::window::{KWindow, KEventReceiver, KEvent};
    ...
    fn receive(&mut self, event : &KEvent) -> bool {
       ...
       match event {
        ...
        KEvent::Keyboard(event) => match event {
            olympus_kleio::window::KEventKeyboard::KeyDown(code) => {
                if *code == 53 {
                    std::process::exit(0);
                } else {
                    println!("Event={:?}", event)
            ...
            olympus_kleio::window::KEventKeyboard::KeyUp(_) => println!("Event={:?}", event),
        },
        KEvent::Mouse(event) => match event {
            olympus_kleio::window::KEventMouse::Moved(_, _) => (),
